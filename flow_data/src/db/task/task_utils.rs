...
pub fn get_tasks_by_project(db: &Db, project_id: u32) -> Result<Vec<Task>> {
    ...
    get_tasks(&mut stmt, [project_id])
}

pub fn get_all_tasks(db: &Db) -> Result<Vec<Task>> {
    let mut stmt = db.0.prepare(task_query::ALL_TASKS)?;
    get_tasks(&mut stmt, [])
}
...
fn get_tasks<P: Params>(stmt: &mut Statement, data: P) -> Result<Vec<Task>> {
    let rows = stmt.query_map(data, |row| {
